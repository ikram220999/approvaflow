<div class=" border-gray-400 rounded-lg p-2">
<div class="border-l-2 mt-0 overflow-scroll h-60">


  <div class="flex h-auto items-center justify-center bg-white  w-full">
    <div class=" border-l-2 border-dashed">
      <div class="relative w-11/12">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          fill="currentColor"
          class="absolute -top-0.5 z-10 ml-3.5 h-7 w-7 rounded-full text-indigo-500"
        >
          <path
            fill-rule="evenodd"
            d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm13.36-1.814a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.14-.094l3.75-5.25z"
            clip-rule="evenodd"
          />
        </svg>
        <div class="ml-14 text-start">
          <h4 class="font-bold text-indigo-500">
            Frontend Development.
          </h4>
          <p class="mt-2 max-w-screen-sm text-sm text-gray-500">
            Maecenas finibus nec sem ut imperdiet. Ut tincidunt est ac
            dolor aliquam sodales. Phasellus sed mauris hendrerit,
            laoreet sem in, lobortis ante.
          </p>
          <span class="mt-1 block text-sm font-semibold text-indigo-500">
            2007
          </span>
        </div>
      </div>

    </div>
  </div>
</div>
</div>